...
export default class Record extends Command {
    ...
    public async exec(message: BMessage, args: string[], voice: VoiceConnection): Promise<any> {
        ...
        await message.success(`I will record ${message.author} for ${timeout / 1000} Seconds`);

        await this.playDing(voice);
        ...
        await this.client.AudioStorage.add({
            commandname: name,
    ...
    private playDing(voice: VoiceConnection): Promise<void> {
