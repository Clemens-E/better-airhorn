...
module.exports = async (client: BClient, message: BMessage): Promise<any> => {
    ...
    if (!cmd) return;
    if (lag() > 20) return message.error('the client is currently not able to process your request. please try again later',
        `process overloaded, ${lag()}ms lag.`)
    ...
    if (cmd.voiceChannel) {
        ...
        voiceConnection = await member.voice.channel.join().catch(() => null);
    ...
    if (cmd.voteLock) {
        const voted = await hasVoted(author.id).catch(() => null);
        if (voted === null) {
